...
(
    job_applied_id int auto_increment unique,
    applicant_id int,
    posted_by_id int,
    job_id int,
    ...
    extra_col1 varchar(255),
    extra_col2 varchar(255),
    primary key (applicant_id,job_id,posted_by_id),
    foreign key (applicant_id) references applicant_account (applicant_id) ON DELETE CASCADE,
    foreign key (posted_by_id) references recruiter_account (recruiter_id) ON DELETE CASCADE,
...
-- Queries needed
 ...
 select aa.applicant_id,aa.first_name,aa.last_name,aa.email,aa.gender,aa.contact_number,aa.registration_date,aa.profileImg,ed.degree_name,ed.university_name,ed.start_date,ed.completion_date,ed.percentage,ed.skill_set,ja.job_applied_id from applicant_account aa JOIN job_applied ja on aa.applicant_id = ja.applicant_id JOIN educational_details ed on aa.applicant_id = ed.applicant_id
 where job_id = ? and ja.selected = false;
